...
const useAccountStore = create<AccountStore>((set) => ({
  ...
  fetchBankAccountData: async () => {
    try {
      ...
      const response = await axios.get<IBankAccountData[]>(
        `${BE_URL}/account/getAllMy`,
        {
  ...
  addBankAccount: async (newAccount: IBankAccountData) => {
    try {
      const BE_URL = import.meta.env.VITE_BE_PORT;
      const res = await axios.post<IBankAccountData[]>(
        `${BE_URL}/account/add`,
        newAccount,
        {
          withCredentials: true,
  ...
  deleteBankAccount: async (id: string) => {
    try {
      const BE_URL = import.meta.env.VITE_BE_PORT;
      const res = await axios.delete<IBankAccountData[]>(
        `${BE_URL}/account/deleteMy/${id}`,
        {
          withCredentials: true,
        }
      );
